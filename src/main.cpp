...
uint256 hashGenesisBlock("0x08a68208361055604ef52eefc3a943b1c0acb4ebaa5579d281c82623b96d6624");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Potatocoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Potatocoin Signed Message:\n";
...
{
    // Potatocoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Potatocoin: 840k blocks in ~4 years
...
//!!!static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Potatocoin: 3.5 days
//!!!static const int64 nTargetSpacing = 2.5 * 60; // Potatocoin: 2.5 minutes
static const int64 nTargetTimespan = 3 * 24 * 60 * 60; // Potatocoin: 3 days !!!每3天调整难度
static const int64 nTargetSpacing = 2 * 60; // Potatocoin: 2 minutes !!!2分钟一个块
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Potatocoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Potatocoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Potatocoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
		...
		hashGenesisBlock = uint256("0x9476e785344d080d72e94830140997de672071d5d1bbb3aac13ecbc76c48f7d2");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Chinese tycoon wants to 'rebuild' New York Times";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1390003200;	// !!!发布时间：2014年01月18日上午 8:00整
        
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2086026951;

        if (fTestNet)
        {
            block.nTime    = 1389052800;// !!!测试时间2014年01月07日上午 8:00整
            block.nNonce   = 385314034;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x3f893856da9821faae0fdf937160e5576e7da08bf6b25108326a64c789ec3e80"));
        //assert(block.hashMerkleRoot == uint256("0x97ddfbbae6be97fd6cdf3e7ca13232a3afff2353e29badfab7f73011edd4ced9"));
        // !!!生成一个Merkle hash的根块哈唏
		// {
//#define _CREATE_GENESIS_BLOCK
#if defined(_CREATE_GENESIS_BLOCK)
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Potatocoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("PotatocoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("PotatocoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static PotatocoinMiner(CWallet *pwallet)
{
    printf("PotatocoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("potatocoin-miner");
    ...
    try { loop {
        ...
        printf("Running PotatocoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("PotatocoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&PotatocoinMiner, pwallet));
}
